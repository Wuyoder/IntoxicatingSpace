window.addEventListener('DOMContentLoaded', () => {
  ...
  axios.get('/api/1.0/user/showlist').then((response) => {
    ...
    for (i = 0; i < 6; i++) {
      ...
      show2 += `<a href="./show.html?${data[i].rss_id}"><img class="show_image" src="${data[i].rss_image}" class="show">
            <div class="show_name">${data[i].rss_title}</div>
            <div class="show_category">${data[i].rss_category_main}</div></a>`;
      show3 += `<a href="./show.html?${data[i].rss_id}"><img class="show_image" src="${data[i].rss_image}" class="show">
            <div class="show_name">${data[i].rss_title}</div>
            <div class="show_category">${data[i].rss_category_main}</div></a>`;
    }
